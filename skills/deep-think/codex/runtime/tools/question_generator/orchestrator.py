...
from tools.question_generator.openclaw_executor import OpenClawStageExecutor
...
EXECUTOR_TRACE_FILENAME = "executor.trace.txt"
EXECUTOR_ERROR_FILENAME = "executor.error.txt"
...
def run_stage(
    ...
    *,
    ...
    trace_path = stage_dir / EXECUTOR_TRACE_FILENAME
    error_path = stage_dir / EXECUTOR_ERROR_FILENAME
    response_schema = None if normalized_stage == "render" else build_stage_response_schema(normalized_stage)
    prompt_text = build_stage_agent_prompt(normalized_stage, prompt_path.read_text(encoding="utf-8"))
    resolved_executor = executor or _resolve_executor(
        executor_backend=executor_backend,
        codex_bin=codex_bin,
        workspace_dir=workspace,
        response_schema_dir=stage_dir,
    )

    try:
        execution = resolved_executor.run_stage_prompt(
            stage=normalized_stage,
            prompt_text=prompt_text,
            timeout_seconds=timeout_seconds,
            response_schema=response_schema,
        )
    except ExecutorInvocationError as exc:
        trace_path.write_text(exc.trace_text or "", encoding="utf-8")
        error_path.write_text(exc.error_text or "", encoding="utf-8")
        record = {
            **prepared,
            "status": "executor_failed",
            "timeout_seconds": timeout_seconds,
            "executor_backend": exc.backend_name,
            "executor_trace_path": str(trace_path),
            "executor_error_path": str(error_path),
    ...
    trace_path.write_text(execution.trace_text or "", encoding="utf-8")
    error_path.write_text(execution.error_text or "", encoding="utf-8")
    try:
        ...
        record = {
            **prepared,
            "status": "response_apply_failed",
            "timeout_seconds": timeout_seconds,
            "executor_backend": execution.backend_name,
            "executor_trace_path": str(trace_path),
            "executor_error_path": str(error_path),
            "failure_reason": "invalid_stage_response",
        }
        if execution.metadata:
            record.update(execution.metadata)
        _write_stage_record(run_dir, normalized_stage, record)
        raise RuntimeError(str(exc)) from exc
...
def _resolve_executor(
    *,
    ...
    if executor_backend == "codex":
        return CodexStageExecutor(
            codex_bin=codex_bin,
            workspace_dir=workspace_dir,
            response_schema_dir=response_schema_dir,
        )
    if executor_backend == "openclaw":
        return OpenClawStageExecutor.from_env()
