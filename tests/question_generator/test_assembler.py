...
class AssemblerTest(unittest.TestCase):
    def test_question_generation_prompt_interpolates_placeholders(self) -> None:
        ...
        self.assertEqual(prompt.count("## Current State"), 1)
        self.assertEqual(prompt.count("## Active Steering"), 1)
    ...
    def test_question_generation_prompt_appends_blocks_without_placeholders(self) -> None:
        ...
        self.assertLess(prompt.index("## Current State"), prompt.index("## Active Steering"))
        self.assertLess(prompt.index("## Active Steering"), prompt.index("## Required Output"))
    ...
    def test_real_stage_templates_assemble_cleanly_with_populated_state(self) -> None:
        ...
        for stage in all_stages:
            with self.subTest(stage=stage):
                ...
                expected_current_state_count = 0 if stage == "routing" else 1
                self.assertEqual(prompt.count("## Current State"), expected_current_state_count)

                expected_steering_count = 0 if stage == "routing" else 1
                self.assertEqual(prompt.count("## Active Steering"), expected_steering_count)
                ...
                if "## Current State" in prompt and "## Active Steering" in prompt:
                    self.assertLess(prompt.index("## Current State"), prompt.index("## Active Steering"))
                if "## Active Steering" in prompt:
                    self.assertLess(prompt.index("## Active Steering"), prompt.index("## Required Output"))
                elif "## Current State" in prompt:
                    self.assertLess(prompt.index("## Current State"), prompt.index("## Required Output"))
                if stage == "routing":
